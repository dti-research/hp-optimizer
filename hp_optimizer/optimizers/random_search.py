...
def sample(space, num_configs):
    """Returns a specified number of configurations drawn randomly
       from the search space

    Arguments:
        space {dict} -- containing function expressions for all
                        hyperparameters to optimize
        num_configs {[type]} -- number of sample configurations
                                to draw from the search space

    Returns:
        list -- nested lists containing all sampled hyperparameter
                configurations and the corresponding values of each
                hyperparameter.
    """
    samples = []
...
def get_model(env, samples, config_num=0):
    """
        Initializes the model to be evaluated, with specified hyperparameter
        values from a given configuration. In this case we want to run the
        cartpole environment using TRPO with a multilayer perceptron as the
        function approximator.


        Parameters:
        -----------
            samples: nested lists containing all sampled hyperparameter
                     configurations and the corresponding values of each
                     hyperparameter.
        ...
        Returns:
        --------
            model: initialized model with specified settings ready to be
                   trained
    """
...
# Evaluation metric is calculated as the mean of the sum of episodic rewards
# for the given model.
...
if __name__ == "__main__":
    # specify number of configurations to sample
    num_configs = 3
    # used to extract resulting metric after training of each config
    config_evals = []
